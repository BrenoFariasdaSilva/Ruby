...
def sorted_alphabetically?(urls)
   names = urls.map { |url| get_name_from_url(url) } # Get the names from the URLs
   names == names.sort # Return true if the names are sorted alphabetically, otherwise false
end

# This method sorts the URLs alphabetically
def sort_urls_alphabetically(urls)
   urls.sort_by { |url| get_name_from_url(url) } # Sort the URLs alphabetically by the name
end
...
def main
   ...
   # Sort the URLs alphabetically if they are not sorted alphabetically
